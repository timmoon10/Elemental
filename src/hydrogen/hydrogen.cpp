...
#include <El-lite.hpp>
#include "El/hydrogen/hydrogen.hpp"
...
hydrogen_comm::hydrogen_comm(int ppm, const El::mpi::Comm world) :
  world_comm(world), grid(nullptr) {
  ...
  //XXXXsplit_models(procs_per_model);
...
hydrogen_comm::~hydrogen_comm() {
  delete grid;
  //XXXEl::mpi::Free(model_comm);
  //XXXEl::mpi::Free(intermodel_comm);
...
---
XXX
void hydrogen_comm::intermodel_sum_matrix(Mat& mat) {
...
void hydrogen_comm::intermodel_sum_matrix(AbsDistMat& mat) {
  allreduce(mat, intermodel_comm, El::mpi::SUM);
}
*/


void hydrogen_comm::allreduce(AbsDistMat& m,
                           const El::mpi::Comm c,
                           El::mpi::Op op,
                           std::type_index t) {
  //XXXbytes_sent += sizeof(DataType) * local_size;
#ifdef HYDROGEN_USES_ALUMINUM
  if (m.LocalHeight() != m.LDim()) {
    std::cerr << "Aluminum does not support allreduces on non-contiguous matrices\n";
    std::terminate();
  }
  ...
  El::AllReduce(m, c, op);
#endif //HYDROGEN_USES_ALUMINUM
 //XXX bytes_received += sizeof(DataType) * local_size * (El::mpi::Size(c) - 1);
}

void hydrogen_comm::nb_allreduce(AbsDistMat& m,
                              const El::mpi::Comm c,
                              Al::request& req,
                              El::mpi::Op op,
                              std::type_index t) {
...
#endif // HYDROGEN_USES_NCCL2
  //XXXbytes_received += sizeof(DataType) * local_size * (El::mpi::Size(c) - 1);
#else
  allreduce(m, c, op);
#endif // HYDROGEN_USES_ALUMINUM
}

void hydrogen_comm::wait(Al::request& req) {
#ifdef HYDROGEN_USES_ALUMINUM
...
bool hydrogen_comm::test(Al::request& req) {
  bool req_test = true;
#ifdef HYDROGEN_USES_ALUMINUM
  if (req.mpi_req != Al::mpi_backend::null_req) {
...
/*
 ---
XXX
void hydrogen_comm::intermodel_broadcast_matrix(Mat& mat, int root) {
  El::Broadcast(mat, intermodel_comm, root);
}

void hydrogen_comm::intermodel_broadcast_matrix(AbsDistMat& mat, int root) {
  El::Broadcast(mat, intermodel_comm, root);
}

void hydrogen_comm::intermodel_barrier() {
  ++num_intermodel_barriers;
  barrier(intermodel_comm);
}

void hydrogen_comm::model_barrier() {
  ++num_model_barriers;
  barrier(model_comm);
}
*/

void hydrogen_comm::global_barrier() {
  //XXX++num_global_barriers;
  barrier(get_world_comm());
}

void hydrogen_comm::barrier(const El::mpi::Comm c) {
  El::mpi::Barrier(c);
}
...
void hydrogen_comm::send(const Mat& mat, int model, int rank) {
  send(mat.LockedBuffer(), mat.Height() * mat.Width(), model, rank);
}

void hydrogen_comm::send(const DistMat& mat, int model, int rank) {
  send(mat.LockedBuffer(), mat.LocalHeight() * mat.LocalWidth(), model, rank);
}

void hydrogen_comm::nb_send(const Mat& mat, int model, int rank,
                         El::mpi::Request<DataType>& req) {
  nb_send(mat.LockedBuffer(), mat.Height() * mat.Width(), model, rank, req);
}

void hydrogen_comm::nb_send(const DistMat& mat, int model, int rank,
                         El::mpi::Request<DataType>& req) {
  nb_send(mat.LockedBuffer(), mat.LocalHeight() * mat.LocalWidth(), model,
          rank, req);
}

void hydrogen_comm::recv(Mat& mat, int model, int rank) {
  recv(mat.Buffer(), mat.Height() * mat.Width(), model, rank);
}

void hydrogen_comm::recv(DistMat& mat, int model, int rank) {
  recv(mat.Buffer(), mat.LocalHeight() * mat.LocalWidth(), model, rank);
}
*/

void hydrogen_comm::recv(Mat& mat) {
  recv(mat.Buffer(), mat.Height() * mat.Width());
}

void hydrogen_comm::recv(DistMat& mat) {
...
/*
 --XXX
 *
void hydrogen_comm::nb_recv(Mat& mat, int model, int rank,
                         El::mpi::Request<DataType>& req) {
  nb_recv(mat.Buffer(), mat.Height() * mat.Width(), model, rank, req);
}

void hydrogen_comm::nb_recv(DistMat& mat, int model, int rank,
                         El::mpi::Request<DataType>& req) {
  nb_recv(mat.Buffer(), mat.LocalHeight() * mat.LocalWidth(), model, rank, req);
}
*/

void hydrogen_comm::nb_recv(Mat& mat, El::mpi::Request<DataType>& req) {
  nb_recv(mat.Buffer(), mat.Height() * mat.Width(), req);
}

void hydrogen_comm::nb_recv(DistMat& mat, El::mpi::Request<DataType>& req) {
...
---
XXX
void hydrogen_comm::intermodel_allreduce(
  ...
  const hydrogen_comm::allreduce_options opts) {
  // Determine which algorithm to actually use.
  hydrogen_comm::allreduce_algorithm algo = opts.algo;
  ...
  default:
    throw hydrogen_exception("intermodel_allreduce: bad algorithm");
    break;
  }
}
*/
...
uint8_t *hydrogen_comm::get_collective_buffer(size_t size, size_t idx) {
  auto buf_iter = collective_bufs.find(size);
  if (buf_iter == collective_bufs.end()) {
    if (idx != 0) {
      std::cerr << "get_collective_buffer: non-contiguous index\n";
      std::terminate();
    }
...
void hydrogen_comm::recursive_doubling_allreduce_pow2(
  const El::mpi::Comm comm, Mat& mat, int max_recv_count,
  std::function<uint8_t *(Mat&, El::IR, El::IR, int&, bool, int)> send_transform,
  std::function<int(uint8_t *, Mat&, bool)> recv_apply_transform,
  const hydrogen_comm::allreduce_options opts) {
  //XXXdouble ar_start = get_time();
  ...
  if (nprocs & (nprocs - 1)) {
    std::cerr << "hydrogen_comm: recursive doubling allreduce requires a power-of-2 number of participating processes\n";
    std::terminate();
  }
  ...
  while (mask < nprocs) {
    ...
    //XXXdouble send_trans_start = get_time();
    int send_size;
    ...
    //XXXar_send_transform_time += get_time() - send_trans_start;
    //XXXbytes_sent += send_size;
    //XXXar_bytes_sent += send_size;
    //XXXdouble sendrecv_start = get_time();
    El::mpi::SendRecv(send_buf, send_size, partner,
                      recv_buf, recv_size, partner, comm);
    //XXXdouble sendrecv_tot = get_time() - sendrecv_start;
    //XXXar_send_time += sendrecv_tot;
    //XXXar_recv_time += sendrecv_tot;
    // Transform and reduce the received data.
    //XXXdouble recv_apply_trans_start = get_time();
    recv_size = recv_apply_transform(recv_buf, mat, is_local);
    //XXXar_recv_apply_transform_time += get_time() - recv_apply_trans_start;
    //XXXbytes_received += recv_size;
    //XXXar_bytes_received += recv_size;
    mask <<= 1;
  }
  //XXXar_time += get_time() - ar_start;
}

void hydrogen_comm::pe_ring_allreduce(
  const El::mpi::Comm comm, Mat& mat, int max_recv_count,
  std::function<uint8_t *(Mat&, El::IR, El::IR, int&, bool, int)> send_transform,
  std::function<int(uint8_t *, Mat&)> recv_transform,
  std::function<int(uint8_t *, Mat&, bool)> recv_apply_transform,
  ...
  for (int outer_step = 1;
    ...
    for (int step = outer_step; step < outer_step + reduces_this_step; ++step) {
      ...
      //XXXdouble recv_tot = get_time() - recv_start;
      //XXXar_recv_time += recv_tot;
      //XXXar_rs_recv_time += recv_tot;
      ...
      //XXXbytes_sent += send_size;
      //XXXar_bytes_sent += send_size;
      //XXXar_rs_bytes_sent += send_size;
      // Post the send.
      //XXXdouble send_start = get_time();
      El::mpi::ISend(send_buf, send_size, dst, comm, send_reqs[reduce_idx]);
      //XXXdouble send_tot = get_time() - send_start;
      //XXXar_send_time += send_tot;
      //XXXar_rs_send_time += send_tot;
    }
    ...
    for (int i = 0; i < reduces_this_step; ++i) {
      ...
      //XXXar_rs_recv_time += recv_tot;
      //XXXdouble recv_apply_trans_start = get_time();
      //int recv_size = recv_apply_transform( recv_buffers[completed_idx], accum_view, local_recvs[completed_idx]);
      //XXXar_recv_apply_transform_time += get_time() - recv_apply_trans_start;
      //XXXbytes_received += recv_size;
      //XXXar_bytes_received += recv_size;
      //XXXar_rs_bytes_received += recv_size;
    }
  ...
  //XXXar_rs_time += get_time() - rs_start;
  // Do a ring allgather.
  //XXXdouble ag_start = get_time();
  ...
  {
   //XXX double send_trans_start = get_time();
    ...
    //XXXar_send_time += sendrecv_tot;
    //XXXar_recv_time += sendrecv_tot;
    //XXXar_ag_send_time += sendrecv_tot;
    //XXXar_ag_recv_time += sendrecv_tot;
    //XXXdouble recv_trans_start = get_time();
    int recv_size = 0;
    if (opts.id_recv) {
    ...
    //XXXar_recv_transform_time += get_time() - recv_trans_start;
    //XXXbytes_received += recv_size;
    //XXXar_bytes_received += recv_size;
    //XXXar_ag_bytes_received += send_size;
    send_size = recv_size;
  }
  ...
  //XXXar_ag_time += get_time() - ag_start;
  //XXXar_time += get_time() - ar_start;
}

void hydrogen_comm::ring_allreduce(
  const El::mpi::Comm comm, Mat& mat, int max_recv_count,
  std::function<uint8_t *(Mat&, El::IR, El::IR, int&, bool, int)> send_transform,
  std::function<int(uint8_t *, Mat&)> recv_transform,
  std::function<int(uint8_t *, Mat&, bool)> recv_apply_transform,
  ...
  for (int step = 0; step < nprocs - 1; ++step) {
    ...
    //XXXar_send_time += sendrecv_tot;
    //XXXar_recv_time += sendrecv_tot;
    //XXXar_rs_send_time += sendrecv_tot;
    //XXXar_rs_recv_time += sendrecv_tot;
    //XXXdouble recv_apply_trans_start = get_time();
  ...
  int send_size;
  {
    ...
    //XXXbytes_received += recv_size;
    //XXXar_bytes_received += recv_size;
    //XXXar_ag_bytes_received += recv_size;
  }
  uint8_t *recv_buf2 = nullptr;
  if (!opts.id_recv) {
    recv_buf2 = get_collective_buffer(max_recv_count, 1);
...
void hydrogen_comm::rabenseifner_allreduce(
  const El::mpi::Comm comm, Mat& mat, int max_recv_count,
  std::function<uint8_t *(Mat&, El::IR, El::IR, int&, bool, int)> send_transform,
  std::function<int(uint8_t *, Mat&)> recv_transform,
  std::function<int(uint8_t *, Mat&, bool)> recv_apply_transform,
  ...
  // This implementation requires a power-of-2 number of processes.
  if (nprocs & (nprocs - 1)) {
    std::cerr << "hydrogen_comm: Rabenseifner allreduce requires a power-of-2 number of participating processes\n";
    std::terminate(); 
  }
  ...
  while (partner_mask > 0) {
    ...
    //XXXbytes_received += recv_size;
    //XXXar_bytes_received += recv_size;
    //XXXar_rs_bytes_received += send_size;
  ...
  while (partner_mask < nprocs) {
    ...
    //XXXar_ag_send_time += sendrecv_tot;
    //XXXar_ag_recv_time += sendrecv_tot;
    //XXX double recv_trans_start = get_time();
    if (opts.id_recv) {
      recv_size = sizeof(DataType) * recv_view.Height() * recv_view.Width();
    } else {
      recv_size = recv_transform(recv_buf, recv_view);
...
void hydrogen_comm::setup_node_comm() {
...
#ifdef EL_HYBRID
void hydrogen_comm::setup_threads() {
...
void hydrogen_comm::reset_threads() {
...
#ifdef HYDROGEN_USES_ALUMINUM
::Al::MPICommunicator* hydrogen_comm::get_al_comm(El::mpi::Comm c,
                                               std::type_index t) {
  ...
  if (comm == nullptr) {
    std::cerr << "Could not get Aluminum communicator\n";
    std::terminate();
  }
  return comm;
...
::Al::ReductionOperator hydrogen_comm::mpi_op_to_al_op(El::mpi::Op op) {
  if (op == El::mpi::SUM) {
    ...
    std::cerr << "Reduction operator not supported in Aluminum\n";
    std::terminate();
  }
}
#endif //HYDROGEN_USES_ALUMINUM
