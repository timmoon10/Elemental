        ...
        Matrix<Real>& scales )
{
    ...
    while( j<numShifts )
    {
        ...
        const Real shiftReal = shifts.GetRealPart(j,0);
        const Real shiftImag = shifts.GetImagPart(j,0);
        const bool shiftIsComplex = ( shiftImag != Real(0) );
        ...
        else
        {
            ...
            const Real smallDiag = Max( ulp*OneAbs(shiftReal), smallNum );
        ...
        Matrix<Real>& scales ) 
{
    ...
    // Parameters to prevent overflow/underflow
    const Real underflow = limits::SafeMin<Real>();
    const Real overflow = limits::Max<Real>();
    const Real ulp = limits::Precision<Real>();
    const Real smallNum = Max( underflow/ulp, Real(1)/(overflow*ulp) );
    ...
    // Check if matrix entries are too large
    DEBUG_ONLY(
      if( MaxNorm(U) >= bigNum )
          LogicError("Entries in matrix are too large");
    )

    // Set scaling factors to 1 and initialize scaling factor updates
    Ones( scales, n, 1 );
    Matrix<Real> scalesUpdate( n, 1 );
    
    // Iterate through matrix diagonal blocks
    // Note: diagonal blocks are chosen so that we don't split any 2x2
    // blocks
    Int kPrev = m;
    while( kPrev>0 )
    {
        // Choose diagonal block so that we don't split any 2x2 blocks
        Int k = Max(kPrev-bsize,0);
        const bool in2x2 = ( k>0 && U.Get(k,k-1) != Real(0) );
        if( in2x2 )
        {
            ++k;
        }
        const Int nb = kPrev-k;

        // Initialize submatrices
        // [ U00 U01 U02 ] [ X00 X01 X02 ]
        // [     U11 U12 ] [     X11 X12 ]
        // [         U22 ] [         X22 ]
        ...
        // TODO: More descriptive names given exploiting quasi-upper-triangular struct
        ...
        if( k > 0 )
        {
            ...
            Gemm( NORMAL, NORMAL, Real(-1), U01, X1, Real(1), X0 );
        }
...
( const ElementalMatrix<Real>& UPre, 
  const ElementalMatrix<Complex<Real>>& shiftsPre,
        ...
        ElementalMatrix<Real>& scalesPre ) 
{
#if 0 // TODO: implement
    DEBUG_CSE
