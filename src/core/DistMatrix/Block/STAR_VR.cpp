...
BDM& BDM::operator=( const DistMatrix<T,MR,STAR,BLOCK,D>& A )
{
    ...
    DistMatrix<T,STAR,VC,BLOCK,D> A_STAR_VC( A_MR_MC );
    A_MR_MC.Empty();
    *this = A_STAR_VC;
    return *this;
}
...
template <typename T, Device D>
int BDM::RowStride() const EL_NO_EXCEPT { return this->Grid().VRSize(); }
...
template <typename T, Device D>
int BDM::RowRank() const EL_NO_EXCEPT { return this->Grid().VRRank(); }
