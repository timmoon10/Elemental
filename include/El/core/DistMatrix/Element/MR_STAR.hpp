...
class DistMatrix<Ring,MR,STAR,ELEMENT,Dev>
...
public:
    ...
    using type = DistMatrix<Ring,MR,STAR,ELEMENT,Dev>;
    using transType = DistMatrix<Ring,STAR,MR,ELEMENT,Dev>;
    using diagType = DistMatrix<Ring,MR,STAR,ELEMENT,Dev>;
    ...
    DistMatrix(DistMatrix<Ring,MR,STAR,ELEMENT,Dev2> const& A);
    ...
    template <Device Dev2,typename=typename std::enable_if<(Dev!=Dev2)&&(IsDeviceValidType<Ring,Dev>::value)&&(IsDeviceValidType<Ring,Dev2>::value)>::type>
    type& operator=(DistMatrix<Ring,MR,STAR,ELEMENT,Dev2> const& A);
