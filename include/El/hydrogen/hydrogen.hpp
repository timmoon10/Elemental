...
//
// hydrogen_comm .hpp .cpp - LBANN communication utilities
////////////////////////////////////////////////////////////////////////////////
#ifndef HYDROGEN_HPP
#define HYDROGEN_HPP
...
#else
using mpi_backend = El::Al::dummy_backend;
#endif // HYDROGEN_USES_ALUMINUM
/// @todo MPI-CUDA backend
#if defined(HYDROGEN_USES_ALUMINUM) && defined(HYDROGEN_USES_NCCL2)
using nccl_backend = ::Al::NCCLBackend;
#else
using nccl_backend = El::Al::dummy_backend;
#endif // defined(HYDROGEN_USES_ALUMINUM) && defined(HYDROGEN_USES_NCCL2)
...
class hydrogen_comm {
 public:
   ...
   * and communicators.
   */
  //xXXvoid split_models(int procs_per_model);
...
XXX
  ...
  //XXXvoid intermodel_sum_matrix(Mat& mat);
  //XXXvoid intermodel_sum_matrix(AbstractDistMatrix& mat);
  /** Broadcast mat over the inter-model communicator starting from root. */
  //XXXvoid intermodel_broadcast_matrix(Mat& mat, int root);
  //XXXvoid intermodel_broadcast_matrix(AbstractDistMatrix& mat, int root);
  /**
...
*/
  ...
  T scatter(T *snd, const El::mpi::Comm c) {
    //XXXbytes_sent += sizeof(T) * (El::mpi::Size(c) - 1);
    T val = {};
...
*/
  ...
  void allreduce(T *snd, int count, T *rcv, const El::mpi::Comm c, El::mpi::Op op = El::mpi::SUM) {
    //XXXbytes_sent += count * sizeof(T);
#ifdef HYDROGEN_USES_ALUMINUM
#ifdef HYDROGEN_ALUMINUM_MPI_PASSTHROUGH
...
#endif //HYDROGEN_ALUMINUM_MPI_PASSTHROUGH
   ...
   *  If HYDROGEN has not been built with Aluminum, then this calls a
   ...
   *  If HYDROGEN has not been built with Aluminum, then this calls a blocking
   *  allreduce.
  ...
  //XXXvoid intermodel_barrier();
  /** Barrier among processes in this model. */
 //XXX void model_barrier();
...
XXX
  ...
  template <typename T>
  void send(const Matrix<T>& mat, int model, int rank);
  ...
  void sendrecv(const T *snd, int send_count, int send_model, int send_rank,
    ...
    //XXXbytes_sent += sizeof(T) * send_count;
...
*/
   ...
   *  the cuDNN manager and make the HYDROGEN communicator responsible
   *  for GPU management.
   */
 ...
 //XXX El::mpi::Comm model_comm;
  /** Communicator for every process with the same model rank. */
  //XXXEl::mpi::Comm intermodel_comm;
